...
namespace MiniGames
{
    class ConnectionDB
    {
        ...
        {
            string sql = "CREATE TABLE Player (IdPlayer INT PRIMARY KEY, UserName VARCHAR(100), BestTimeSolitaire INT, NbWinSolitaire INT, NbDefeatSolitaire INT, NbWinBataille INT, NbDefeatBataille INT, NbWinMorpion INT, NbDefeatMorpion INT)";
        ...
        {
            string sql = "CREATE TABLE Score (IdScore INT PRIMARY KEY, FkPlayer INT, FOREIGN KEY(FkPlayer) REFERENCES Player(IdPlayer))";
            SQLiteCommand command = new SQLiteCommand(sql, m_dbConnection);
            command.ExecuteNonQuery();
        }
